...
router.get("/ebooks", auth, async (req, res) => {
  try {
    ...
    if (query) {
      ...
      ebooks = await Ebook.find().populate("book_id name authors available cover_path section");
    }
    res.json(ebooks);
  } catch (err) {
    console.error(err.message);
...
router.put('/subscription', auth, async (req, res) => {
  const { subscription, request_available } = req.body;

  if (!subscription && request_available === undefined) {
    return res.status(400).json({ message: 'Provide either subscription or request_available to update.' });
  }

  try {
    const updateFields = {};
    if (subscription) updateFields.subscription = subscription;
    if (request_available !== undefined) updateFields.request_available = request_available;
