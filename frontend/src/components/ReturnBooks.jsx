...
const ReturnBooks = () => {
  ...
  const fetchBorrowedBooks = async (_student) => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:5001/api/librarian/issued-books/student/${_student._id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleReturnConfirm = async () => {
    try {
      ...
      await axios.put(
        `http://127.0.0.1:5001/api/librarian/issued-books/${returnBook._id}`,
        { returnDate, status },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleReturnAll = async (studentid) => {
    ...
    try {
      await axios.put(
        `http://127.0.0.1:5001/api/librarian/students/${studentid}/issued-books`,
        {
